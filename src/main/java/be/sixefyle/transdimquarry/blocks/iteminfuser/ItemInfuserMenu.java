...
public class ItemInfuserMenu extends AbstractContainerMenu {
    public final ItemInfuserBlockEntity blockEntity;
    ...
    public ItemInfuserMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
        super(MenuRegister.ITEM_INFUSER.get(), id);

        blockEntity = (ItemInfuserBlockEntity) entity;
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            addSlot(new SlotItemHandler(handler, 0, 10, 29));
            addSlot(new SlotItemHandler(handler, 1, 10, 59));
            addSlot(new SlotItemHandler(handler, 2, 131, 44));
            addSlot(new SlotItemHandler(handler, 3, 151, 44));
        });
    ...
    private static final int TE_INVENTORY_SLOT_COUNT = ItemInfuserBlockEntity.CONTAINER_SIZE;  // must be the number of slots you have!
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, BlockRegister.ITEM_INFUSER.get());
    }
